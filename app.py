...
@app.route('/single-user-home')
def single_home():
    return render_template('single-user-home.html')
    ...
    if request.method == 'POST':
        ...
        if utils.authenticate(role, email, password):
            ...
            if(role=='Restaurant Owner'):
                ...
                return render_template('single-user-home.html',name=name,role=userRole)
...
@app.route('/single-register', methods=['GET', 'POST'])
def singleRegister():
    if request.method == 'POST':
        role = request.form['role']
        ...
        details = {
            "name": name, "email": email, "address": address, "contact": contact, "date": curr_date
        }
    ...
    return render_template('single-user-register.html')
...
def donation():
    donation_data = utils.getHistoryData()
...
@app.route('/ask-donations', methods=['GET', 'POST'])
def askDonations():
    request_data = utils.getTodaysRequest()
    ...
    if request.method == 'POST':
        ...
        utils.save_details(role, emailAdd, request_data, donation=False)
        # utils.donation_received_email(donation_data)

    return render_template('ask-donations.html',request_data=request_data)
...
def viewRequests():
    request_data = utils.getRequestsData()
    length = 0
    if request_data:
        length = len(request_data)

    return render_template('view-requests.html',request_data=request_data,length=length)
