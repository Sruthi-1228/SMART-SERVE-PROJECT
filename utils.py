...
def save_details(role, emailAdd, details, donation=False):
    ...
    if donation:
        ...
        data["requestData"][emailAdd]=details
...
def getRequestsData():
    requestData = list(load_data()["requestData"].values())
    return requestData
...
def getTodaysRequest():
    requestData = getRequestsData()
    todayData = []
    curr_date = datetime.now().strftime("%d-%m-%Y")

    for i, dData in enumerate(requestData):
        if dData['date']==curr_date:
            todayData.append(dData)
